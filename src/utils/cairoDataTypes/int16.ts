 ...
 * new CairoInt16(-5).toBigInt(); //     -5n
 * new CairoInt16('-5').toBigInt(); //   -5n
 * new CairoInt16('-0x5').toBigInt(); // -5n     a sign, not the text "-0x5"
 * ```
 */
export class CairoInt16 {
   ...
   * leading minus sign — and as UTF-8 text otherwise. Text therefore only fits here up to 2 ASCII
   * characters : one more already makes a number past 32767.
   ...
   * // result = ["24930"]
   * const result2 = new CairoInt16('-5').toApiRequest();
   ...
   * const result3 = CairoInt16.__processData('-0x5');
   ...
   * an array, a number with a decimal part, `'0x'`, and a value outside [-32768, 32767]. A text string reaches
