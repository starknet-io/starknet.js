...
export class CairoTypeOption extends CairoType {
  ...
  /* CairoType instance representing the content of a Cairo option. */
  ...
  public toApiRequest(): string[] {
    const result = [this.isVariantSome ? '0' : '1'];
  ...
  private decomposeSome(strategyDecompose: AllowArray<ParsingStrategy>): any {
    ...
    if (responseParser) {
      return responseParser(content as CairoType, strategies);
    }
   ...
   * @example
   * ```typescript
   * const myOption = new CairoTypedOption(3, "core::option::Option::<core::integer::u8>", hdParsingStrategy, CairoOptionVariant.Some);
