 ...
 */
export class LedgerSigner210<Transport extends Record<any, any> = any>
  ...
  implements SignerInterface
{
   ...
   * @param {LedgerPathCalculation} [pathFunction=getLedgerPathBuffer210]
   * defines the function that will calculate the path. By default `getLedgerPathBuffer210` is selected.
   *
   * If you are using APP v2.1.0 with an account created with the v1.1.1, you need to use :
   * ```typescript
   * const myLedgerSigner = new LedgerSigner210(myNodeTransport, 0, undefined, getLedgerPathBuffer111);
   * ```
   * @example
   * ```typescript
   * import TransportNodeHid from "@ledgerhq/hw-transport-node-hid";
   * const myNodeTransport = await TransportNodeHid.create();
   * const myLedgerSigner = new LedgerSigner210(myNodeTransport, 0);
   * ```
   */
  constructor(
    transport: Transport,
    ...
    pathFunction: LedgerPathCalculation = getLedgerPathBuffer210
  ) {
  ...
  public async signTransaction(
    ...
    console.log('sender =', transactionsDetail.walletAddress);
    ...
    if (Object.values(ETransactionVersion2).includes(transactionsDetail.version as any)) {
      ...
      console.log('sign txV2 Ledger - txH calculated by SNJS =', { msgHash });
    ...
    if (Object.values(ETransactionVersion3).includes(transactionsDetail.version as any)) {
      ...
      console.log('sign txV3 Ledger - txH calculated by SNJS =', { msgHash });
  ...
  async signTxV1(
    ...
    // APDU 0 for path
    console.log('4-0- Path =', this.pathBuffer);
    ...
    */
    ...
    console.log('maxFee =', txDetails.maxFee);
    const chainIdBuf: Uint8Array = this.convertBnToLedger(txDetails.chainId);
    console.log('txDetails.nonce =', txDetails.nonce);
    ...
    console.log('4-1-', { dataBuf }, '\n', dataBuf.subarray(100));
    const res410 = await this._transporter.send(Number('0x5a'), 4, 1, 0, Buffer.from(dataBuf));
    console.log({ res410 });
    // APDU 2 = Nb of calls
    const nbCallsBuf: Uint8Array = this.convertBnToLedger(calls.length);
    console.log('4-2', { nbCallsBuf });
    const res0 = await this._transporter.send(Number('0x5a'), 4, 2, 0, Buffer.from(nbCallsBuf));
    console.log({ res0 });
    // APDU 3 = Calls
    ...
    for (const call of calls) {
      ...
      console.log('4-3-0-', calldatas[0], '\n', calldatas[0].subarray(100));

      const res1 = await this._transporter.send(Number('0x5a'), 4, 3, 0, Buffer.from(calldatas[0]));
      console.log({ res1 });
    ...
    console.log('resp Ledger =', respSign);
  ...
  async signTxV3(
    ...
    // APDU 0 for path
    console.log('3-0- Path =', this.pathBuffer);
    ...
    */
    ...
    console.log('3-1-', { dataBuf }, '\n', dataBuf.subarray(100));
    const res310 = await this._transporter.send(Number('0x5a'), 3, 1, 0, Buffer.from(dataBuf));
    console.log({ res310 });
    ...
    console.log('3-2-', { paymasterBuf }, '\n', paymasterBuf.subarray(100));
    const res320 = await this._transporter.send(Number('0x5a'), 3, 2, 0, Buffer.from(paymasterBuf));
    console.log({ res320 });
    ...
    console.log('3-3-', { accountDeployDataBuf }, '\n', accountDeployDataBuf.subarray(100));
    const res330 = await this._transporter.send(
      Number('0x5a'),
      3,
      3,
      0,
      Buffer.from(accountDeployDataBuf)
    );
    console.log({ res330 });

    // APDU 4 = Nb of calls
    const nbCallsBuf: Uint8Array = this.convertBnToLedger(calls.length);
    console.log('3-4', { nbCallsBuf });
    const res0 = await this._transporter.send(Number('0x5a'), 3, 4, 0, Buffer.from(nbCallsBuf));
    console.log({ res0 });
    // APDU 5 = Calls
    let respSign: Uint8Array = new Uint8Array(0);
    // eslint-disable-next-line no-restricted-syntax
    for (const call of calls) {
      const calldatas: Uint8Array[] = this.encodeCall(call);
      console.log('3-5-0-', calldatas[0], '\n', calldatas[0].subarray(100));
      const res1 = await this._transporter.send(Number('0x5a'), 3, 5, 0, Buffer.from(calldatas[0]));
      console.log({ res1 });
      if (calldatas.length > 1) {
        calldatas.slice(1).forEach(async (part: Uint8Array) => {
 ...
 * @example
 * ```typescript
 * const result = getLedgerPathBuffer210(0);
 ...
 * ```
 */
export function getLedgerPathBuffer210(
  accountId: number,
