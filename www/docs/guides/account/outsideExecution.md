...
Nevertheless, the account that will execute the transaction does not need to be SNIP-9 compatible; it just needs to have enough fees to pay the transaction.
:::
...
:::warning
You can use the string `"ANY_CALLER"` as content of the `caller` property. To use with care, as anybody that gets your `OutsideTransaction` object can execute it.
:::
