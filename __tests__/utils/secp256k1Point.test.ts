...
describe('secp256k1Point cairo type test', () => {
  const myCallDataAccount = new CallData(CONTRACTS.AccountEthOz090.sierra.abi);
  const myCallDataTest = new CallData(CONTRACTS.TestEthPubKey.sierra.abi);
  const ethPubKey =
  ...
  test('secp256k1Point is an u512', () => {
    ...
    expect(() => {
      ...
      // the message is CairoSecp256k1Point's own now: the class is what refuses the value, where
      // a separate validation pass used to speak first
