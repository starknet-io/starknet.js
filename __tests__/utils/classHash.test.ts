...
describe('Hash Tester', () => {
  ...
  describe('Compute CompiledClassHash & ClassHash Cairo1', () => {
    test('Hello - CompiledClassHash', () => {
      const compiledClassHash = hash.computeCompiledClassHash(CONTRACTS.Hash.casm, '0.13.1');
    ...
    test('Hello - CompiledClassHash Cairo2.6.0 Sierra1.5.0', () => {
      const compiledClassHash = hash.computeCompiledClassHash(CONTRACTS.Hello260.casm, '0.13.1');
      expect(compiledClassHash).toEqual(
    ...
    test('Hello - ClassHash', () => {
      const classHash = hash.computeSierraContractClassHash(CONTRACTS.Hash.sierra);
