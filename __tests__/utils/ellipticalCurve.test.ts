import { Signature, getPublicKey, getStarkKey, pedersen, sign, verify } from 'micro-starknet';
...
test('hashMessage()', () => {
  ...
  const { r, s } = Signature.fromHex(sign(hashMsg, privateKey));
...
test('verify signed message()', () => {
  ...
  expect(verify(signature, hashMsg, pubKey)).toBe(true);
});
